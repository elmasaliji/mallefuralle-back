...
const Navigation = ({ onOpenTickets }) => {
  ...
  // Offset scroll da naslov ne ulazi ispod fixed nav-a
  const scrollToSection = (id) => {
    const element = document.getElementById(id);
    if (!element) return;

    const navOffset = 96; // prilagodi ako treba (npr. 80 ili 110)
...
// Sticky Ticket Button Component
...
// Hero Section Component (video background FIXED)
const HeroSection = ({ onOpenTickets }) => {
  const [isMuted] = useState(true);

  return (
    <section id="hero" className="relative h-screen min-h-[700px] flex items-center justify-center overflow-hidden">
      {/* Video Background - FIXED (ostaje iza dok skrolas) */}
      <div className="fixed inset-0 z-0 pointer-events-none">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-[#d40e7b]/10 via-transparent to-[#9621ff]/10"></div>
      </div>
      ...
      {/* Floating Social Media Icons - Only Instagram & TikTok */}
...
function App() {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Navigation + Sticky Ticket ONLY ONCE */}
      ...
      <main className="relative overflow-x-hidden">
        ...
        {/* Sekcije ispod hero-a normalno skrolaju preko fixed video pozadine */}
